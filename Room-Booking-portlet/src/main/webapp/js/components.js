(function() {
	...
	AUI.add(
		...
		function(A) {
			...
			var STR_CALENDAR_LIST = 'room-list';
			...
			var TPL_CALENDAR_LIST_ITEM = new A.Template(
				'<tpl for="rooms">',
					'<div class="', CSS_CALENDAR_LIST_ITEM, '">',
						'<div class="', CSS_CALENDAR_LIST_ITEM_COLOR, '" {[ parent.rooms[$i].get("visible") ? ', '\'style="background-color:\'', STR_PLUS, 'parent.rooms[$i].get("color")', STR_PLUS, '";border-color:"', STR_PLUS, 'parent.rooms[$i].get("color")', STR_PLUS, '";\\""', ' : \'', STR_BLANK, '\' ]}></div>',
							'<span class="', CSS_CALENDAR_LIST_ITEM_LABEL, '">{[LString.escapeHTML(parent.rooms[$i].getDisplayName())]}</span>',
						'<div class="', CSS_CALENDAR_LIST_ITEM_ARROW, '"><i class="', CSS_ICON_CARET_DOWN, '"></i></div>',
					'</div>',
				'</tpl>'
			);

			var RoomList = A.Component.create(
				{
					ATTRS: {
						rooms: {
							setter: '_setRooms',
						...
						strings: {
							value: {
								emptyMessage: Liferay.Language.get('no-rooms-selected')
							}
						}
					},
					...
					prototype: {
						...
						bindUI: function() {
							...
							instance.on('scheduler-room:colorChange', instance._onRoomColorChange, instance);
							instance.on('scheduler-room:visibleChange', instance._onRoomVisibleChange, instance);
						...
						add: function(room) {
							...
							var rooms = instance.get('rooms');

							rooms.push(room);

							instance.set('rooms', rooms);
						},
						...
						getRoomByNode: function(node) {
							var instance = this;

							var rooms = instance.get('rooms');

							return rooms[instance.items.indexOf(node)];
						},

						getRoomNode: function(room) {
							var instance = this;

							var rooms = instance.get('rooms');

							return instance.items.item(AArray.indexOf(rooms, room));
						},
						...
						remove: function(room) {
							...
							if (rooms.length > 0) {
								var index = AArray.indexOf(rooms, room);

								if (index > -1) {
									AArray.remove(rooms, index);
								}
							}
						...
						_clearRoomColor: function(room) {
							var instance = this;

							var node = instance.getRoomNode(room);
						...
						_onRoomColorChange: function(event) {
							var instance = this;

							var target = event.target;

							if (target.get('visible')) {
								instance._setRoomColor(target, event.newVal);
							}
						},

						_onRoomVisibleChange: function(event) {
							var instance = this;

							var target = event.target;

							if (event.newVal) {
								instance._setRoomColor(target, target.get('color'));
							}
							else {
								instance._clearRoomColor(target);
							}
						},
						...
						_onClick: function(event) {
							...
							if (target) {
								...
								instance.activeItem = instance.getRoomByNode(activeNode);
								...
								var room = instance.getRoomByNode(event.currentTarget);

								room.set('visible', !room.get('visible'));
							}
						},
						...
						_onHoverOut: function(event) {
							...
							var room = instance.getRoomByNode(currentTarget);

							if (!room.get('visible')) {
								instance._clearRoomColor(room);
							}
						...
						_onHoverOver: function(event) {
							...
							if (!room.get('visible')) {
								instance._setRoomColor(room, room.get('color'));
							}
						},
						...
						_setRoomColor: function(room, val) {
							var instance = this;
						...
						_setRooms: function(val) {
							...
							AArray.each(
								...
								function(item, index) {
									...
									if (!A.instanceOf(item, Liferay.SchedulerRoom)) {
										room = new Liferay.SchedulerRoom(item);
										val[index] = room;
										console.log("!A.instanceOf(item, Liferay.SchedulerRoom)");
									}

									room.addTarget(instance);

									room.set('scheduler', scheduler);
								}
							);
	...
	AUI.add(
		'liferay-room-simple-color-picker',
		function(A) {
			var	AArray = A.Array;
			var Lang = A.Lang;
	...
	AUI.add(
		'liferay-room-reminders',
		function(A) {
			var Lang = A.Lang;

			var TPL_REMINDER_SECTION = '<div class="room-portlet-reminder-section form-inline">' +
				'<label class="checkbox">' +
					'<input class="room-portlet-reminder-check" id="{portletNamespace}reminder{i}" name="{portletNamespace}reminder{i}" type="checkbox" <tpl if="!disabled">checked="checked"</tpl> />' +
				'</label>' +
			...
			var Reminders = A.Component.create(
				{
					ATTRS: {
						...
						values: {
							...
							value: [
								{
									interval: 10,
									type: Liferay.RoomUtil.NOTIFICATION_DEFAULT_TYPE
								},
								{
									interval: 60,
									type: Liferay.RoomUtil.NOTIFICATION_DEFAULT_TYPE
					...
					prototype: {
						...
						bindUI: function() {
							...
							boundingBox.delegate('change', instance._onChangeCheckbox, '.room-portlet-reminder-check', instance);
						},
		...
		function(A) {
			...
			Liferay.DatePickerUtil = {
				syncUI: function(form, fieldName, date) {
					...
					var datePicker = Liferay.component(Liferay.RoomUtil.PORTLET_NAMESPACE + fieldName + 'datePicker');

					if (datePicker) {
						datePicker.room.deselectDates();
						datePicker.room.selectDates(date);
	...
	AUI.add(
		'liferay-room-message-util',
		function(A) {
			var Lang = A.Lang;
			var LString = Lang.String;

			var TPL_MESSAGE_UPDATE_ALL_INVITED = '<p class="room-portlet-confirmation-text">' +
				Liferay.Language.get('invited-users-will-be-notified') +
			'</p>';

			Liferay.RoomMessageUtil = {
						...
						queue.add(
							{
								...
								autoContinue: false,
								context: instance,
								fn: instance._queueableQuestionUserRoomOnly,
								timeout: 0
							}
						);
					}
				...
				_queueableQuestionUpdateAllInvited: function(data) {
					...
					if (answers.cancel) {
						...
						Liferay.RoomMessageUtil.confirm(
				...
				_queueableQuestionUserRoomOnly: function(data) {
					var instance = this;
					...
					if (answers.cancel) {
						...
						var content = [
							'<p class="room-portlet-confirmation-text">',
							Lang.sub(
								Liferay.Language.get('you-are-about-to-make-changes-that-will-only-affect-your-room-x'),
								[LString.escapeHTML(data.roomName)]
							),
							'</p>'
						].join(STR_BLANK);
